...
def node(state: AgentState) -> AgentState:
    ...
    if skill_name == "filesystem":
        if "list all python files" in description.lower():
            tactical_plan = [
                "filesystem:list_files(pattern=*.py)",
                "filesystem:list_files(pattern=**/*.py)"
            ]
        else:
        ...
        if "extract function definitions" in description.lower():
            tactical_plan = [
                "filesystem:read_file(path={file})" for file in state.get("python_files", [])
            ]
        ...
        if "complexity issues" in description.lower():
            tactical_plan = [
                "code_parser:analyze_complexity(functions={functions})"
            ]
        else:
        ...
        if "summary report" in description.lower():
            tactical_plan = [
                "filesystem:write_file(path=analysis_report.md,content={report})"
            ]
        else:
