...
def test_list_tools_success(sse_server_url):
    """Test successfully listing tools from the localtime MCP server via SSE."""
    transport = SSETransport()

    try:
        transport.start("sse", [sse_server_url])
...
def test_get_current_time_success(sse_server_url):
    """Test successfully getting current time from the localtime MCP server via SSE."""
    transport = SSETransport()
